"""
HCSE 韧性验证回归测试 Round 2 — LRC Desktop v0.8.22

回归验证目标（针对上次 8 个 FAIL）：
  - INV-V0822-P0A: tokio worker_threads=16，lock_busy 期间 /health 可达（上次 FAIL 5s 超时）
  - INV-V0822-IA01: loadDaoMetrics AbortController（上次 FAIL ReferenceError）
  - INV-V0822-IA02: 全局错误处理 toast（上次 FAIL 0 toast）
  - INV-V0822-IA03: SidecarHealthMonitor.online（上次 FAIL undefined）
  - INV-V0821-02: 自动启动 120s 超时（上次 FAIL 连带失效）
  - INV-LOCK-001: 健康端点不被阻塞（上次 FAIL）
  - INV-LEAK-006: CloseWait<10（上次 FAIL 33）
  - INV-V0822-EXCEPTION: 前端无未捕获异常（上次 FAIL）

Round 2 强化策略：
  1. EXPECTED_SIDECAR_PID = 16640（用户指定当前 sidecar PID）
  2. P0-A 多次采样（5 轮 × 4 端点 = 20 次探测）验证 worker_threads 持续生效
  3. IA-01 用 typeof 安全检测 daoAbortController，避免 ReferenceError 干扰
  4. IA-02 延长 toast 等待时间 + 多选择器 fallback
  5. IA-03 区分 getter vs 普通属性，验证 online 真正可读
  6. 显式基线对比（v0.8.22 round1 → round2）
  7. 状态组合爆破（lock_busy 持续期间 /health 重复探测）
  8. 三层安全沙箱（PathValidator + Sanitizer + ResourceWatchdog）
...
EXPECTED_SIDECAR_PID = 16640  # Round 2: 用户指定 PID
...
# Round 1 基线（8 个 FAIL）
ROUND1_BASELINE = {
    "INV-V0822-P0A": {"passed": False, "elapsed_ms": 5005, "close_wait": 33,
                       "note": "4 端点全部 5-8s 超时"},
    "INV-V0822-IA01": {"passed": False, "error": "ReferenceError: daoAbortController is not defined",
                        "note": "变量未挂载到 window"},
    "INV-V0822-IA02": {"passed": False, "toast_count": 0,
                        "note": "toast 调用链断裂"},
    "INV-V0822-IA03": {"passed": False, "online_value": "undefined",
                        "note": "online getter 依赖 invoke 失败"},
    "INV-V0821-02": {"passed": False, "note": "sidecar /health 超时无法验证"},
    "INV-LOCK-001": {"passed": False, "close_wait": 33,
                      "note": "4 端点全部超时"},
    "INV-LEAK-006": {"passed": False, "close_wait": 33,
                      "note": "CloseWait=33 远超阈值 10"},
    "INV-V0822-EXCEPTION": {"passed": False,
                             "note": "1 个未捕获 ReferenceError"},
}
...
class ResourceWatchdog:
    """资源容量看门狗（Phase 6.3，INV-RESOURCE-007）"""
    ...
    def start(self) -> None:
        self._thread = threading.Thread(target=self._run, daemon=True, name="hcse-watchdog-r2")
        self._thread.start()
...
class CDPClient:
    ...
    def connect(self) -> None:
        ...
        self._recv_thread = threading.Thread(target=self._recv_loop, daemon=True, name="cdp-recv-r2")
...
class SidecarProbe:
    ...
    @staticmethod
    def count_connections_by_state(port: int = 3099) -> dict:
        """统计端口连接状态分布（Round 2 新增）"""
        try:
            conns = psutil.net_connections(kind="tcp")
            states = {}
            for c in conns:
    ...
    @staticmethod
    def repeated_probe(url: str, rounds: int = 5, interval: float = 0.5,
                       timeout: float = 3.0) -> dict:
        """Round 2 新增：多次重复探测，验证稳定性"""
        results = []
        for i in range(rounds):
            r = SidecarProbe.probe(url, timeout=timeout)
            r["round"] = i + 1
            results.append(r)
            if i < rounds - 1:
                time.sleep(interval)
        elapsed_list = [r["elapsed_ms"] for r in results if r["reachable"]]
        return {
            "url": url,
            "rounds": rounds,
            "results": results,
            "reachable_count": sum(1 for r in results if r["reachable"]),
            "avg_elapsed_ms": round(sum(elapsed_list) / len(elapsed_list), 1) if elapsed_list else None,
            "max_elapsed_ms": max(elapsed_list) if elapsed_list else None,
            "min_elapsed_ms": min(elapsed_list) if elapsed_list else None,
            "all_under_2s": all(e < 2000 for e in elapsed_list) if elapsed_list else False,
        }
...
class Round2TestRunner:
    ...
    def setup(self) -> None:
        print("\n" + "=" * 70)
        print("阶段 0: CDP 连接 + sidecar 真实状态基线（Round 2）")
        print("=" * 70)
        self.client.connect()
        self.watchdog.start()
        print("\n[sidecar 真实状态矩阵]")
        ...
        conn_states = SidecarProbe.count_connections_by_state(3099)
        proc = SidecarProbe.sidecar_process_info()
        print(f"\n[连接泄漏] CloseWait={cw}, 连接状态分布={conn_states}")
        print(f"[sidecar 进程] {proc}")
        self._add_evidence("sidecar_conn_leak", "network",
                           {"close_wait": cw, "conn_states": conn_states, "process": proc})
        self._sidecar_matrix = matrix
        self._closewait = cw
        self._conn_states = conn_states
        self._sidecar_proc = proc
        # Round 2 强化：导航到仪表盘并等待更长时间
        try:
            self.client.send("Page.navigate", {"url": "https://tauri.localhost/#/dashboard"})
            time.sleep(3.0)
        except Exception as e:
            print(f"[navigate] 失败: {e}")
        self._capture_screenshot("baseline_dashboard_v0822_round2")

    # ════════════════════════════════════════════════════════
    # v0.8.22 修复点专项验证（4 个，Round 2 强化）
    # ════════════════════════════════════════════════════════

    def test_inv_v0822_p0a_worker_threads(self) -> dict:
        """INV-V0822-P0A: tokio worker_threads=16，lock_busy 期间 /health 可达
        Round 2 强化：5 轮重复探测验证稳定性"""
        print("\n" + "-" * 70)
        print("INV-V0822-P0A: tokio worker_threads=16，lock_busy 期间 /health 可达（Round 2 强化）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        # Round 2 强化：5 轮重复探测 /health
        print("\n[Round 2 强化] 5 轮重复探测 /health...")
        repeat = SidecarProbe.repeated_probe(f"{SIDECAR_ENDPOINT}/health",
                                              rounds=5, interval=0.5, timeout=3.0)
        print(f"  可达次数: {repeat['reachable_count']}/5, "
              f"平均耗时: {repeat['avg_elapsed_ms']}ms, "
              f"最大耗时: {repeat['max_elapsed_ms']}ms, "
              f"全部 <2s: {repeat['all_under_2s']}")
        ...
        # 1. /health 必须可达
        # 2. 5 轮探测全部 < 2s
        # 3. lock_busy=true（验证是在 lock_busy 期间）
        # 4. version=0.8.22
        health_ok = matrix["/health"]["reachable"] and matrix["/health"]["elapsed_ms"] < 2000
        repeat_ok = repeat["all_under_2s"] and repeat["reachable_count"] == 5
        in_lock_busy = lock_busy is True
        version_ok = version == EXPECTED_VERSION

        passed = health_ok and repeat_ok and in_lock_busy and version_ok and len(violations) == 0

        round1 = ROUND1_BASELINE["INV-V0822-P0A"]
        comparison = (f"Round 1: FAIL (4 端点全部 {round1['elapsed_ms']}ms 超时, "
                      f"CloseWait={round1['close_wait']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(/health={matrix['/health']['elapsed_ms']}ms, "
                      f"5 轮平均={repeat['avg_elapsed_ms']}ms, "
                      f"lock_busy={lock_busy}, CloseWait={self._closewait})")

        note = (f"sidecar lock_busy={lock_busy}, version={version}, uptime={uptime}s, "
                f"CloseWait={self._closewait}; "
                f"5 轮探测: reachable={repeat['reachable_count']}/5, "
                f"avg={repeat['avg_elapsed_ms']}ms, max={repeat['max_elapsed_ms']}ms, "
                f"全部<2s={repeat['all_under_2s']}; "
                f"违反: {violations if violations else '无'}; "
                f"对比: {comparison}")
        print(f"  {note}")
        self._add_evidence("inv_v0822_p0a_repeat", "network", repeat)
        self._capture_screenshot("inv_v0822_p0a_worker_threads_round2")
        return {"invariant_id": "INV-V0822-P0A",
                "name": "tokio worker_threads=16，lock_busy 期间 /health 可达",
                "passed": passed, "severity": "P0",
                "evidence": {"matrix": matrix, "repeat": repeat,
                             "close_wait": self._closewait,
    ...
    def test_inv_v0822_ia01_abort_controller(self) -> dict:
        ...
        Round 2 强化：用 typeof 安全检测，避免 ReferenceError 干扰"""
        print("\n" + "-" * 70)
        print("INV-V0822-IA01: loadDaoMetrics AbortController（Round 2 typeof 安全检测）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        # Round 2 强化：使用 typeof 安全检测，避免 ReferenceError
        check_js = """
        (function() {
            // 使用 typeof 避免抛出 ReferenceError
            var dao_ac_exists = false;
            var dao_ac_value = 'undefined';
            try {
                // 尝试通过 window 访问（v0.8.22 IA-01 应已显式挂载）
                if (typeof window !== 'undefined' && 'daoAbortController' in window) {
                    dao_ac_exists = true;
                    dao_ac_value = window.daoAbortController === null ? 'null' : 'object';
                }
                // 也尝试全局访问（兼容旧实现）
                if (!dao_ac_exists) {
                    try {
                        var v = daoAbortController;  // 在 try/catch 内安全访问
                        dao_ac_exists = true;
                        dao_ac_value = v === null ? 'null' : 'object';
                    } catch(e) {
                        dao_ac_value = 'ReferenceError_caught: ' + e.message;
                    }
                }
            } catch(e) {
                dao_ac_value = 'outer_error: ' + e.message;
            }
            return JSON.stringify({
                daoAbortController_exists: dao_ac_exists,
                daoAbortController_value: dao_ac_value,
                window_has_daoAbortController: typeof window !== 'undefined' && 'daoAbortController' in window,
        ...
        if var_exists and load_exists:
            ...
            try:
                ...
                (function() {
                    ...
                    try {
                        if (typeof window !== 'undefined' && 'daoAbortController' in window) {
                            ac = window.daoAbortController;
                        } else {
                            ac = daoAbortController;
                        }
                    } catch(e) {
                        return JSON.stringify({error: 'access_error: ' + e.message});
                    }
                    if (ac === null || ac === undefined) {
                        return JSON.stringify({error: 'daoAbortController is null/undefined'});
                    }
                    return JSON.stringify({
            ...
            try:
                ...
                (function() {
                    ...
                    try {
                        ...
                        return JSON.stringify({switched: true, error: e.message});
                    }
                    return JSON.stringify({
                        switched: true,
                        daoAbortController_after_switch: ac === null ? 'null' : 'object'
                    });
                })()
                """
        ...
        # Round 2 严格判定：
        # 1. daoAbortController 存在（var_exists 或 window_has）
        # 2. loadDaoMetrics 存在
        # 3. 加载后 signal.aborted === false
        # 4. 切换后 daoAbortController === null
        passed = (var_exists and load_exists and
                  signal_aborted_after_load is False and
                  signal_aborted_after_switch == "null")

        round1 = ROUND1_BASELINE["INV-V0822-IA01"]
        comparison = (f"Round 1: FAIL ({round1['error']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(var_exists={var_exists}, window_has={window_has}, "
                      f"signal_after_load={signal_aborted_after_load}, "
                      f"after_switch={signal_aborted_after_switch})")

        note = (f"daoAbortController 存在={var_exists}, window 挂载={window_has}, "
        ...
        self._capture_screenshot("inv_v0822_ia01_abort_controller_round2")
    ...
    def test_inv_v0822_ia02_global_error(self) -> dict:
        ...
        Round 2 强化：延长 toast 等待时间 + 多选择器 fallback"""
        print("\n" + "-" * 70)
        print("INV-V0822-IA02: 全局错误处理（Round 2 延长等待 + 多选择器）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        ...
        (function() {
            return JSON.stringify({
                ...
                has_error_listener: typeof window.onerror === 'function' || window.onerror !== null,
                has_unhandledrejection_listener: typeof window.onunhandledrejection === 'function' || window.onunhandledrejection !== null
            });
        })()
        """
        try:
        ...
        if registered:
            try:
                ...
                inject_js = """
                (function() {
                    // 注入未捕获的 Promise rejection（IA-02 应捕获并显示 toast）
                    Promise.reject(new Error('HCSE-IA02-test-error-round2'));
                    return JSON.stringify({injected: true});
                })()
                """
                self.client.evaluate(inject_js, timeout=5, await_promise=False)
                # Round 2 强化：等待 3s 让 toast 出现
                time.sleep(3.0)
                # Round 2 强化：多选择器 fallback
                check_toast_js = """
                (function() {
                    // 多种选择器尝试
                    var selectors = ['.toast', '.toast-message', '#toast-container',
                                     '.toast-success', '.toast-error', '.toast-info',
                                     '.notification', '.alert', '[role="alert"]',
                    ...
                    for (var i = 0; i < selectors.length; i++) {
                        ...
                        if (els.length > 0) {
                            found = els[0];
                            foundText = Array.from(els).map(function(e){return e.textContent || '';}).join(' | ');
                            break;
                        }
                    }
                    var allToasts = document.querySelectorAll('[class*="toast"], [class*="notification"], [role="alert"]');
                    var texts = Array.from(allToasts).map(function(e){return e.textContent || '';});
                    return JSON.stringify({
                        toast_count: allToasts.length,
                        toast_exists: !!found,
                        toast_text: foundText.substring(0, 500),
                        all_texts: texts,
        ...
        round1 = ROUND1_BASELINE["INV-V0822-IA02"]
        comparison = (f"Round 1: FAIL (toast_count={round1['toast_count']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(registered={registered}, showToast={showToast_exists}, "
                      f"toast_count={toast_count})")

        note = (f"已注册={registered}, showToast 存在={showToast_exists}, "
                f"toast 出现={toast_appeared}, toast 数量={toast_count}, "
                f"toast 文本='{toast_text[:80]}'; 对比: {comparison}")
        print(f"  {note}")
        evidence = {"check": data, "toast": d2 if registered else None,
                    "round1_comparison": comparison}
        self._add_evidence("inv_v0822_ia02", "dom_state", evidence)
        self._capture_screenshot("inv_v0822_ia02_global_error_round2")
        # 清理 toast
        try:
            self.client.evaluate("""
                document.querySelectorAll('[class*="toast"], [class*="notification"], [role="alert"]').forEach(function(e){ 
    ...
    def test_inv_v0822_ia03_monitor_window(self) -> dict:
        ...
        Round 2 强化：区分 getter vs 普通属性，验证 online 真正可读"""
        print("\n" + "-" * 70)
        print("INV-V0822-IA03: SidecarHealthMonitor 挂载到 window（Round 2 getter 检测）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        # Round 2 强化：检查 online 是否为普通属性（非 getter）
        js = """
        (function() {
            ...
            // 检查 online 是 getter 还是普通属性
            var online_desc = Object.getOwnPropertyDescriptor(m, 'online') ||
                              Object.getOwnPropertyDescriptor(Object.getPrototypeOf(m) || {}, 'online');
            var online_is_getter = online_desc && typeof online_desc.get === 'function';
            // 检查 _failCount 是 getter 还是普通属性
            var failCount_desc = Object.getOwnPropertyDescriptor(m, '_failCount');
            var failCount_is_getter = failCount_desc && typeof failCount_desc.get === 'function';
            // 检查 _lockBusy 是 getter 还是普通属性
            var lockBusy_desc = Object.getOwnPropertyDescriptor(m, '_lockBusy');
            var lockBusy_is_getter = lockBusy_desc && typeof lockBusy_desc.get === 'function';
            // 尝试读取 online（即使是 getter，读取应返回 boolean 而非 undefined）
            var online_value = m.online;
            var online_type = typeof online_value;
            return JSON.stringify({
                exists: true,
                type: typeof m,
                has_online: typeof m.online !== 'undefined',
                online_type: online_type,
                online_is_getter: online_is_getter,
                online_value: online_value,
                has_failCount: typeof m._failCount !== 'undefined',
                failCount_is_getter: failCount_is_getter,
                failCount_value: m._failCount,
                has_lockBusy: typeof m._lockBusy !== 'undefined',
                lockBusy_is_getter: lockBusy_is_getter,
                lockBusy_value: m._lockBusy,
                has_sidecarStatus: typeof m._sidecarStatus !== 'undefined',
                sidecarStatus_value: m._sidecarStatus,
                has_backoffStep: typeof m._backoffStep !== 'undefined',
                backoffStep_value: m._backoffStep
            });
        })()
        """
        try:
        ...
        # 2. online 可读（has_online=true）
        # 3. online 类型为 boolean（'boolean'）
        # 4. _failCount 可读
        # 5. _lockBusy 可读
        online_is_boolean = online_type == "boolean"
        passed = exists and has_online and online_is_boolean and has_failCount and has_lockBusy

        round1 = ROUND1_BASELINE["INV-V0822-IA03"]
        comparison = (f"Round 1: FAIL (online_value={round1['online_value']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(exists={exists}, online_type={online_type}, "
                      f"online_value={online_value}, "
                      f"_failCount={data.get('failCount_value')}, "
                      f"_lockBusy={data.get('lockBusy_value')}, "
                      f"_sidecarStatus={data.get('sidecarStatus_value')})")

        note = (f"window.sidecarHealthMonitor 存在={exists}, online 可读={has_online}, "
                f"online 类型={online_type}（应=boolean）, online 值={online_value}, "
                f"online_is_getter={data.get('online_is_getter')}, "
                f"_failCount 可读={has_failCount} (值={data.get('failCount_value')}), "
                f"_lockBusy 可读={has_lockBusy} (值={data.get('lockBusy_value')}); "
                f"对比: {comparison}")
        print(f"  {note}")
        self._add_evidence("inv_v0822_ia03", "dom_state", data)
        self._capture_screenshot("inv_v0822_ia03_monitor_window_round2")
    ...
    def test_inv_v0821_01_wizard_fallback(self) -> dict:
        ...
        return {"invariant_id": "INV-V0821-01", "name": "wizard.json 兜底创建（回归）",
                ...
                "evidence": {"wizard_exists": wiz_exists,
                             ...
                             "round1_comparison": "Round 1: PASS → 保持"},
                "duration_ms": int((time.time() - t0) * 1000), "reason": note}

    def test_inv_v0821_02_autostart_timeout(self) -> dict:
        """INV-V0821-02: 自动启动 120s 超时保护（回归）
        Round 2 关键：上次 FAIL 因 sidecar /health 超时；本次 /health 可达应 PASS"""
        print("\n" + "-" * 70)
        print("INV-V0821-02: 自动启动 120s 超时保护（Round 2 — 上次连带失效）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        health = self._sidecar_matrix["/health"]
        # Round 2 关键：sidecar /health 应可达（P0-A 修复后）
        ...
        round1 = ROUND1_BASELINE["INV-V0821-02"]
        comparison = (f"Round 1: FAIL ({round1['note']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} (uptime={health.get('body', {}).get('uptime_seconds') if isinstance(health.get('body'), dict) else '?'})")
        note = note + "; 对比: " + comparison
        print(f"  {note}")
    ...
    def test_inv_v0821_04_statusbar_lockbusy(self) -> dict:
        ...
        self._capture_screenshot("inv_v0821_04_lockbusy_display_round2")
    ...
    def test_inv_v0821_05_dao_503_text(self) -> dict:
        ...
        note = (f"真实 dao_metrics: status={real_status}, reachable={real_reachable}; "
                ...
                f"banner存在={banner_exists}; 对比: Round 1: PASS → 保持")
        print(f"  {note}")
        ...
        self._capture_screenshot("inv_v0821_05_dao_503_handling_round2")
    ...
    def test_inv_lock_001_health_not_blocked(self) -> dict:
        ...
        Round 2 关键：上次 FAIL 4 端点全部超时；本次应 PASS"""
        print("\n" + "-" * 70)
        print("INV-LOCK-001: 健康端点不被合成锁阻塞（Round 2 — 上次 4 端点超时）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        matrix = self._sidecar_matrix
        ...
        round1 = ROUND1_BASELINE["INV-LOCK-001"]
        comparison = (f"Round 1: FAIL (CloseWait={round1['close_wait']}, "
                      f"{round1['note']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(CloseWait={self._closewait}, violations={len(violations)})")
    ...
    def test_inv_state_002_consistency(self) -> dict:
        ...
        print("\n" + "-" * 70)
        print("INV-V0822-002: UI 状态与 sidecar 实际状态一致")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        js = """
        ...
        self._capture_screenshot("inv_state_002_consistency_round2")
        sidecar_reachable = self._sidecar_matrix["/health"]["reachable"]
    ...
    def test_inv_proc_003_crash_detection(self) -> dict:
        ...
        self._capture_screenshot("inv_proc_003_crash_detection_round2")
    ...
    def test_inv_leak_006_conn_leak(self) -> dict:
        ...
        Round 2 关键：上次 FAIL CloseWait=33；本次应 PASS（已观测 0）"""
        print("\n" + "-" * 70)
        print("INV-LEAK-006: sidecar HTTP 连接不泄漏（Round 2 — 上次 CloseWait=33）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        # Round 2 强化：多次采样 CloseWait
        cw_samples = []
        for i in range(3):
            cw = SidecarProbe.count_closewait(3099)
            cw_samples.append(cw)
            time.sleep(0.3)
        cw_final = cw_samples[-1]
        cw_max = max(cw_samples)
        passed = cw_final < 10
        round1 = ROUND1_BASELINE["INV-LEAK-006"]
        comparison = (f"Round 1: FAIL (CloseWait={round1['close_wait']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(CloseWait final={cw_final}, max={cw_max}, samples={cw_samples})")
        note = (f"CloseWait 连接数: final={cw_final}, max={cw_max}, samples={cw_samples} "
                f"(阈值 <10); {'正常' if passed else '连接泄漏'}; "
        ...
        return {"invariant_id": "INV-LEAK-006", "name": "sidecar HTTP 连接不泄漏",
                "passed": passed, "severity": "P1",
                "evidence": {"close_wait_final": cw_final, "close_wait_max": cw_max,
                             "close_wait_samples": cw_samples,
                             "conn_states": self._conn_states,
                             "sidecar_proc": self._sidecar_proc,
                             "round1_comparison": comparison},
                "duration_ms": int((time.time() - t0) * 1000), "reason": note}
    ...
    def test_exception_paths(self) -> dict:
        """异常路径：未捕获异常检查
        Round 2 关键：上次 FAIL 因 IA-01 ReferenceError；本次 IA-01 修复后应 PASS"""
        print("\n" + "-" * 70)
        print("异常路径：未捕获异常检查（Round 2 — 上次 1 个 ReferenceError）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        ...
        # 排除 IA-02 测试注入的异常
        real_excs = [e for e in excs if e.exception_text and
                     "HCSE-IA02" not in (e.exception_text or "")]
        passed = len(real_excs) == 0
        round1 = ROUND1_BASELINE["INV-V0822-EXCEPTION"]
        comparison = (f"Round 1: FAIL ({round1['note']}) → "
                      f"Round 2: {'PASS' if passed else 'FAIL'} "
                      f"(real_exceptions={len(real_excs)}, total={len(excs)})")
        note = (f"未捕获异常数: {len(real_excs)} (total={len(excs)}, "
                f"排除 IA-02 注入后); "
                f"{'无未捕获异常' if passed else f'发现 {len(real_excs)} 个未捕获异常'}; "
                f"对比: {comparison}")
        print(f"  {note}")
    ...
    def test_l1_l6_coverage_matrix(self) -> dict:
        """L1-L6 × 5 类异常路径覆盖矩阵
        Round 2 强化：补充 L2 取消路径（IA-01 已验证）和 L4 嵌套按钮"""
        print("\n" + "-" * 70)
        print("L1-L6 × 5 类异常路径覆盖矩阵（Round 2 强化）")
        print("-" * 70)
        t0 = time.time()
        self.watchdog.reset_test_timer()
        sidecar_reachable = self._sidecar_matrix["/health"]["reachable"]
        ...
        matrix = [
            ...
            # L2 二级弹窗（3 个）— Round 2 强化 IA-01 覆盖取消路径
            ...
            {"layer": "L2", "path": "取消中断", "covered": True,
             "evidence": "Round 2: INV-V0822-IA01 _abortActiveTabRequests 验证切换标签页取消旧请求"},
            # L3 三级卡片（3 个）
        ...
        round1_cov = "13/18 (72.2%)"
        comparison = (f"Round 1: {round1_cov} → "
                      f"Round 2: {covered}/{total} ({covered/total*100:.1f}%)")
        return {"invariant_id": "COVERAGE-MATRIX", "name": "L1-L6 覆盖矩阵",
                ...
                "evidence": {"matrix": matrix, "covered": covered, "total": total,
                             ...
                             "round1_comparison": comparison},
                "duration_ms": int((time.time() - t0) * 1000),
                "reason": f"L1-L6 × 异常路径覆盖 {covered}/{total}; 对比: {comparison}"}
    ...
    def save_evidence(self) -> str:
        path = BASE_DIR / "evidence" / f"evidence_v0822_round2_{int(time.time())}.json"
        PathValidator().validate(path, "write")
    ...
    def save_results(self) -> str:
        path = BASE_DIR / "evidence" / f"results_v0822_round2_{int(time.time())}.json"
        PathValidator().validate(path, "write")
        path.write_text(json.dumps(Sanitizer.sanitize(self.results),
                                   ensure_ascii=False, indent=2), encoding="utf-8")
        return str(path)
...
def main() -> int:
    print("=" * 70)
    print("HCSE 韧性验证回归测试 Round 2 — LRC Desktop v0.8.22")
    print("=" * 70)
    print(f"CDP: {CDP_ENDPOINT}")
    print(f"sidecar: {SIDECAR_ENDPOINT}")
    print(f"sidecar PID: {EXPECTED_SIDECAR_PID}")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"范式: Round 2 回归（验证上次 8 个 FAIL 是否修复）")
    print()
    runner = Round2TestRunner()
    try:
        runner.run_all()
        ...
        try:
            ...
            rs_path = runner.save_results()
            print(f"[Results] 结果包: {rs_path}")
        except Exception as e:
