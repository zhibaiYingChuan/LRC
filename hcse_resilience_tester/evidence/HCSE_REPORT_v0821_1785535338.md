...
|-----------|------|--------|------|----------|------|
| INV-V0821-01 | wizard.json 兜底创建 | P0 | FAIL | 165 | sidecar 未运行: {'error': 'Unexpected token \'<\', "<!DOCTYPE "... is not valid JSO |
| INV-V0821-02 | 120s 自动启动超时保护 | P0 | FAIL | 4 | JS 异常: SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON |
| INV-V0821-03 | switch_project 120s 超时保护 | P0 | PASS | 1 | Tauri 桥接可用，switch_project 超时保护已就位 |
| INV-V0821-04 | 状态栏 lockBusy 紫色显示 | P1 | FAIL | 1151 | lock-busy类=False, 后台合成中文本=False (故障注入:_lockBusy=true) |
| INV-V0821-05 | dao metrics 503 lock_busy 处理 | P1 | FAIL | 11674 | CDP 命令超时 (10s): Runtime.evaluate |
...
graph TD
    ...
    INV_V0821_01 --> C1["根因: sidecar 未运行: {'error': 'Unexpected token \'<\', '<!DOCTYPE '"]
    B --> INV_V0821_02[INV-V0821-02: 120s 自动启动超时保护]
    INV_V0821_02 --> C1["根因: JS 异常: SyntaxError: Unexpected token '<', '<!DOCTYPE '... is"]
    ...
    INV_V0821_05 --> C1["根因: CDP 命令超时 (10s): Runtime.evaluate"]
    C1 --> D[修复建议]
    D --> E1[1. 检查源码对应行]
    D --> E2[2. 注入故障复现]
    D --> E3[3. 回归测试循环]
