"""
LRC LongMemEval 公平检索能力评估 v3
=====================================
设计目标：在遵守 LongMemEval 基准测试规则下，公平、客观地测出 LRC 的检索基本功。

公平性原则：
  1. 不利用任何 ground truth 信息（has_answer, answer_session_ids）
  2. 所有记忆 importance 统一为 5（不差异化）
  3. tags 只含自然属性（session_id, date, role, turn_idx, level）
  4. 启用 LLM 查询翻译（DeepSeek API）
  5. 支持 fast（TF-IDF + 洛书几何）和 deep（深度语义检索）两种模式

测量的 LRC 检索基本功：
  - TF-IDF 检索（recall 函数）
  - 洛书几何编码（luoshu_encoder）
  - LLM 查询翻译器（llm_translator）
  - 八卦分类、衰减机制、标签匹配

与 v1/v2 的区别：
  - v1: 仅会话级注入，importance=5（基线）
  - v2: Turn 级注入 + has_answer=8（利用 ground truth，不公平）
  - v3: Turn 级注入 + 统一 importance=5（公平，不利用 ground truth）
"""
...
class LRCStdioClient:
    ...
    def start(self):
        ...
        r = self._call("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "lrc-eval-v3", "version": "3.0.0"},
        }, timeout=30)
        if "error" in r:
    ...
    def recall(self, query: str, top_k: int = 10, project: Optional[str] = None,
               lrc_mode: str = "fast", focus_depth: int = 1) -> str:
        """检索记忆，返回原始文本
        ...
        Args:
            ...
            top_k: 返回数量
            project: 项目过滤
            lrc_mode: "fast"（关键词匹配）或 "deep"（深度语义检索）
            focus_depth: deep 模式下的聚焦深度（1-3）
        """
        args = {"query": query, "top_k": top_k, "lrc_mode": lrc_mode}
        if project:
            args["project"] = project
        if lrc_mode == "deep":
            args["focus_depth"] = focus_depth
        result = self._call("tools/call", {
            "name": "recall",
            "arguments": args,
        }, timeout=120)  # deep 模式可能更慢，超时放宽到 120s
        if "error" in result:
            return ""
...
# ============================================================
# v3 公平注入策略：不利用 ground truth，统一 importance
# ============================================================

def build_memories_fair(haystack_sessions, haystack_dates, haystack_session_ids, project_name):
    """v3 公平注入策略：会话级 + Turn 级双重注入，统一 importance=5

    公平性原则：
    1. 不使用 has_answer 字段（ground truth）
    2. 所有记忆 importance=5（统一，不差异化）
    3. tags 只含自然属性（session_id, date, role, turn_idx, level）
    ...
    for session, date_str, sid in zip(haystack_sessions, haystack_dates, haystack_session_ids):
        ...
        if len(full.strip()) >= 20:
            ...
            batch.append({
                ...
                "importance": 5,  # 公平：统一 importance
            })
            n_sessions += 1

        # === Turn 级记忆（importance=5，公平：不使用 has_answer）===
...
def evaluate_retrieval(
    ...
    """评估 LRC 真实 sidecar 的检索精度（v3 公平策略）"""

    mode_label = f"v3-{lrc_mode}" + (f"-d{focus_depth}" if lrc_mode == "deep" else "")
    ...
    print(f"策略: {mode_label} (公平评估，统一 importance=5，不利用 ground truth)", flush=True)
    print(f"检索模式: lrc_mode={lrc_mode}" + (f", focus_depth={focus_depth}" if lrc_mode == "deep" else ""), flush=True)
    ...
    for idx, instance in enumerate(dataset):
        ...
        try:
            ...
            # 步骤 1: 注入记忆（v3 公平策略：会话级 + Turn 级，统一 importance=5）
            t0 = time.time()
            project_name = f"lme_{question_id}"
            batch, stats = build_memories_fair(
            ...
            if verbose and idx < 3:
                print(f"  注入: {stats['total']} 条 (会话={stats['n_sessions']}, "
                      f"turn={stats['n_turns']}) ({t1-t0:.1f}s)", flush=True)

            time.sleep(0.5)

            # 步骤 2: recall 检索（支持 fast/deep 模式）
            t0 = time.time()
            raw_text = client.recall(
                query=question, top_k=top_k, project=f"lme_{question_id}",
                lrc_mode=lrc_mode, focus_depth=focus_depth
            )
            t1 = time.time()
            ...
            if verbose and idx < 5:
                print(f"  recall ({lrc_mode}): {len(recalled)} 条 ({t1-t0:.2f}s)", flush=True)
            ...
            # 注意：这里评估时使用 has_answer 是合法的（用于判定是否命中），
            # 但注入时不使用 has_answer（不利用 ground truth 设置 importance）
            turn_hit = False
            turn_rr = 0.0
            answer_contents = []
    ...
    output_lines.append(f"LRC LongMemEval 检索精度评估报告（{mode_label} 公平评估）")
    output_lines.append("=" * 70)
    ...
    output_lines.append(f"  检索模式: lrc_mode={lrc_mode}" + (f", focus_depth={focus_depth}" if lrc_mode == "deep" else ""))
    output_lines.append(f"  注入策略: 会话级 + Turn 级，统一 importance=5（不利用 ground truth）")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LRC LongMemEval 公平检索能力评估 v3")
    ...
    parser.add_argument("--data-dir", default=r"G:\code-memory\data_eval_v3")
    ...
    parser.add_argument("--mode", choices=["fast", "deep"], default="fast",
                        help="检索模式: fast（TF-IDF+洛书几何）或 deep（深度语义检索）")
    parser.add_argument("--focus-depth", type=int, default=1, choices=[1, 2, 3],
                        help="deep 模式下的聚焦深度（1-3）")
