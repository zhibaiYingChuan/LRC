"""
LRC v0.5.6 HotpotQA 检索精度全面评估脚本
...
HotpotQA 数据集特征（与 MS MARCO / NQ 的差异）：
1. 多跳问答数据集：每个查询恰好有 2 个相关文档（需要跨文档推理）
   - MS MARCO / NQ：通常 1 个相关文档
   - HotpotQA：恰好 2 个相关文档（多跳推理特征）
2. 查询最长且最复杂：平均 92 字符（比较性、桥接性问题）
   - MS MARCO：~30 字符（关键词查询）
   - NQ：~48 字符（自然语言问题）
   - HotpotQA：~92 字符（多跳推理问题）
3. 文档有 title 字段（Wikipedia 段落，与 NQ 类似）
4. BM25 基线最高：NDCG@10 ≈ 0.633（多跳推理有明确答案文档）

LRC v0.5.6 全面发挥的三种检索能力：
1. TF-IDF 检索（词边界匹配修复后）：contains_word 替代 contains
2. 洛书几何编码：remember 时自动编码，recall 时自动几何距离加权
3. LLM 查询翻译器：将多跳问题翻译为答案可能包含的关键词
...
用法示例：
  python lrc_hotpotqa_eval.py --mode tfidf --num-docs 500 --num-queries 100
  python lrc_hotpotqa_eval.py --mode llm --llm-api "openai:sk-xxx:deepseek-chat:https://api.deepseek.com/v1"
  python lrc_hotpotqa_eval.py --mode both --llm-api "openai:sk-xxx:deepseek-chat:https://api.deepseek.com/v1"
"""
import os
import sys
import json
...
class LRCStdioClient:
    ...
    def start(self):
        ...
        resp = self._call("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "hotpotqa-eval-client", "version": "0.5.6"}
        }, timeout=60)
...
def load_corpus_with_sampling(corpus_path, relevant_docs, num_random, seed=42):
    ...
    HotpotQA 适配：文档内容包含 title + text（Wikipedia 段落标题+正文）
    返回: (corpus_dict, total_lines)
    corpus_dict: {pid: {"title": ..., "text": ...}}
    """
    random.seed(seed)
...
def generate_report(all_metrics, all_results, num_docs, num_relevant, output_path, llm_enabled):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# LRC v0.5.6 HotpotQA 检索精度评估报告\n\n")
        f.write(f"**评估日期**: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        f.write(f"**LRC 版本**: v0.5.6\n\n")
        f.write(f"**数据集**: HotpotQA (BEIR test split)\n\n")
        f.write(f"**文档数量**: {num_docs}（相关: {num_relevant}）\n\n")
        f.write("---\n\n")

        f.write("## 1. 评估背景\n\n")
        f.write("### 1.1 HotpotQA 数据集特征\n\n")
        f.write("HotpotQA 是多跳问答数据集，需要跨多个 Wikipedia 文档推理才能回答问题。\n\n")
        f.write("**HotpotQA 与 MS MARCO / NQ 的关键差异**：\n\n")
        f.write("| 维度 | MS MARCO | Natural Questions | HotpotQA |\n")
        f.write("| :--- | :--- | :--- | :--- |\n")
        f.write("| 查询类型 | 关键词查询 | 自然语言问题 | 多跳推理问题 |\n")
        f.write("| 文档来源 | Web 段落 | Wikipedia 段落 | Wikipedia 段落 |\n")
        f.write("| 文档结构 | 仅 text | title + text | title + text |\n")
        f.write("| 每查询相关文档 | 通常 1 个 | 1-4 个（平均 1.22） | 恰好 2 个（多跳） |\n")
        f.write("| 查询平均长度 | ~30 字符 | ~48 字符 | ~92 字符 |\n")
        f.write("| BM25 基线 NDCG@10 | 0.184 | 0.305 | 0.633 |\n\n")
        f.write("**HotpotQA 对 LRC 的挑战**：\n")
        f.write("- 多跳推理：需要找到 2 个相关文档（而非 1 个），召回难度更大\n")
        f.write("- 查询最长最复杂：比较性问题（\"Were X and Y of the same nationality?\"）\n")
        f.write("- 桥接性问题：需要跨文档推理，TF-IDF 只能做词汇匹配\n")
        f.write("- LLM 查询翻译器应更有效（将多跳问题翻译为多个答案关键词）\n\n")
        ...
        f.write("- HotpotQA 文档内容 = title + text（充分利用 Wikipedia 段落标题）\n\n")
        f.write("---\n\n")
        ...
        f.write("HotpotQA 的 BM25 基线 NDCG@10 ≈ 0.633（从 523 万文档中检索）\n\n")
        for m in all_metrics:
            diff = (m["mrr"] / 0.633 - 1) * 100
        ...
        f.write(f"- **HotpotQA 数据集适配**: 文档内容包含 title + text，多跳推理场景评估\n")
        if llm_enabled:
            f.write(f"- **LLM 查询翻译器**: 在多跳推理问题场景下，将问题翻译为多个答案关键词\n")


def main():
    parser = argparse.ArgumentParser(description="LRC v0.5.6 HotpotQA 检索精度全面评估")
    parser.add_argument("--exe", default="G:/rust-target/release/code-memory-server.exe",
                        help="LRC sidecar 二进制路径")
    parser.add_argument("--data-dir", default="G:/BEIR/lrc_hotpotqa_data",
                        help="LRC 数据目录")
    ...
    parser.add_argument("--top-k", type=int, default=10,
                        help="检索 top-k（默认 10）")
    parser.add_argument("--output", default="G:/BEIR/results/LRC_HOTPOTQA_REPORT.md",
                        help="输出报告路径")
    parser.add_argument("--log-dir", default="G:/BEIR/results",
                        help="日志目录")
    parser.add_argument("--dataset-dir", default="G:/BEIR/datasets/hotpotqa",
                        help="HotpotQA 数据集目录")
    parser.add_argument("--split", default="test",
                        help="评估使用的 split（HotpotQA 使用 test split）")
    args = parser.parse_args()

    # 加载查询和 qrels
    print(f"加载 HotpotQA {args.split} split...", flush=True)
    ...
    print(f"  相关文档数: {len(relevant_docs)}（每查询恰好 2 个，多跳推理特征）", flush=True)
    ...
    # 准备注入的记忆
    # HotpotQA 适配：文档内容 = title + text（充分利用 Wikipedia 段落标题）
    project_name = "hotpotqa_eval"
    memories = []
    for pid in selected_docs:
        ...
        # HotpotQA 适配：内容 = "标题: 正文"（让 LRC 能同时匹配标题和正文）
    ...
    for mode_label, llm_api in modes_to_test:
        ...
        # 打印结果
        print(f"\n{'=' * 60}", flush=True)
        print(f"LRC v0.5.6 HotpotQA 检索精度评估结果", flush=True)
        print(f"{'=' * 60}", flush=True)
        print(f"评估模式: {mode_label}", flush=True)
        ...
        # BM25 基线对比
        print(f"\nBM25 基线（HotpotQA test）: NDCG@10 ≈ 0.633", flush=True)
        diff = (metrics['mrr'] / 0.633 - 1) * 100
        status = "优于" if diff > 0 else "低于"
        print(f"LRC vs BM25: {status} BM25 ({diff:+.1f}%)", flush=True)

        # 保存日志
        os.makedirs(args.log_dir, exist_ok=True)
        log_file = os.path.join(args.log_dir, f"hotpotqa_{mode_label.split('(')[0].strip().replace(' ', '_').lower()}.log")
        with open(log_file, "w", encoding="utf-8") as f:
            f.write(f"LRC v0.5.6 HotpotQA 检索精度评估\n")
            f.write(f"{'=' * 60}\n")
