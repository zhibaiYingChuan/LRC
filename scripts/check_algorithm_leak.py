...
====================================================================
★职责边界（v0.9.9 澄清，避免误读其保护范围）
====================================================================

**本脚本防的是什么**：受保护的**专有算法实现**（结构算子的具体公式、
卦映射矩阵、调度权重等）被**复制粘贴**进 Apache 2.0 覆盖的文件里 ——
那会让这些实现脱离 DaoTi Research License 的约束。

**本脚本不防、也不该防什么**：八卦/洛书/道体等**术语与数据结构本身**。
经实测取证（2026-09-19）：若把扫描范围无差别扩大为"除 src/engine/ 外的
全部源码"，会命中 **2039 处**（`八卦编码` 821、`洛书算法` 806、
`道枢映射` 368、`剪枝算法` 32），遍布 `memory_store.rs`、`graph_store.rs`、
`memory_state_machine.rs` 等 —— 这些并非泄露，而是**产品功能本身**：
符号层的推理与状态机**必须**持有八卦/洛书的数据结构与类型定义。

⇒ 因此扫描范围**有意**限定为下列 4 个"纯公开层"文件。它们的共同特征是
**不承载符号层数据结构**，一旦出现受保护术语即为异常（大概率是误粘贴）。

**真正的许可边界不在本脚本，而在**：
  · `LICENSE` §2.2 —— 声明 `src/engine/` 整体适用 DaoTi Research License，
    且**编译产物中的受保护算法部分同样适用**（即：进安装包是许可内的正常分发，
    不是"泄露"）；
  · `README.md` 的「道体（Daoti）边界」小节 —— 向用户如实说明哪些随包、
    哪些不随包、各自适用什么许可。
"""
