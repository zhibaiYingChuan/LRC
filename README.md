...
把 Loong Recall 接入 IDE 之后，你的 AI 助手就获得了两个关键能力：

1. **语义搜索代码库** — 用自然语言描述你想找的代码，而不是靠盲猜文件名
2. **跨会话记忆** — 让 AI 助手记住你的偏好、决策、项目约定，下次对话自动延续
...
③ memory_store.rs — 调用 `recall(query, filter)`
        ...
        ├─→ 从持久化层加载所有记忆
        ├─→ 按类型/项目/标签/重要性过滤
        ├─→ 对查询文本和每条记忆内容做语义相似度匹配（使用 FastEncoder）
        ├─→ 应用衰减因子（长期未访问的记忆自动降权）
        └─→ 返回 Top-K 记忆，按衰减后相似度排序
        │
        ▼
① server.rs — 格式化结果（记忆 ID、类型、标签、匹配度），返回响应
```
...
| **③ 记忆存储层** | `src/memory_store.rs` | 记忆领域聚合根 | 封装所有记忆 CRUD 操作和检索逻辑。`recall()` 不仅做语义匹配，还自动计算衰减因子（指数衰减模型），让长期不用的记忆自然降权。`archive_expired()` 将过期记忆迁入冷存储（`archive.json`），保持活跃记忆库轻量。 |
