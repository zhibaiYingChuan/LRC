...
| **项目记忆** | `.loong-recall/data/memories.json` | 否 | 你通过 `remember` 工具写入的所有记忆，以 JSON 文件存储在项目目录下。这是你的私有数据。 |
| **嵌入向量缓存** | `.loong-recall/cache/embedding_cache.json` | 否 | Smart Match 模式下，编码后的向量缓存到此文件，用于下次启动秒加载。仅在本地磁盘。 |
