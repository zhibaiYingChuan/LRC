...
| 层级 | 范围 | 许可证 | 权限 |
|------|------|--------|------|
| **L1 公开层** | `src/chunker.rs`, `src/server.rs`, `src/bin/` | Apache 2.0 | 完全开放，可商用、可修改、可分发 |
| **L2 受保护层** | `src/engine/` | DaoTi Research License v1.0 | 源码可见，仅限研究/审计，禁止逆向/商用/训练竞品 |
| **L3 编译层** | `code-memory-server` 二进制 | 双重保护 | Rust 编译天然混淆 + 许可证约束 |

**关于 L2 受保护层**：`src/engine/` 目录下的文件包含从道体道枢层衍生的核心编码与检索算法。这些文件在仓库中**可见但受限制**——您可以阅读和学习接口设计，但禁止逆向工程、商业再分发和训练竞争模型。

公开层代码（切分器、MCP 协议服务、CLI 入口）完全开放，欢迎贡献。
