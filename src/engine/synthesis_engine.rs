...
impl SynthesisEngine {
    ...
    ///
    /// ## ★保留理由（2026-09-18 审查明确）
    ///
    /// 本方法**无生产调用方**，但**有 4 个单测在用**（见文件末 `tests` 模块），
    /// 它们是 Jaccard 合成路径的**唯一测试载体**。删除会连带删掉这些测试，
    /// 使该路径彻底失去覆盖。
    ///
    /// ⇒ 判定为「**测试专用入口**」而非死代码：保留，但**显式标注**用途，
    ///   避免下次审计又把它当成"零调用点"重复讨论。
    ///   生产路径请走三阶段：`synthesis_snapshot` + `plan_jaccard` +
    ///   `apply_synthesis_plan`（避免计算阶段持锁）。
    ///
    /// 返回本次新生成的合成记忆数量。
