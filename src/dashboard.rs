...
// v0.9.7（GLOBAL_CODE_REVIEW_REPORT P1-6）：错误类型统一为 LrcError。
...
pub fn open_dashboard(url: &str) -> LrcResult<()> {
    webbrowser::open(url).map_err(|e| LrcError::unsupported(format!("无法打开浏览器: {e}")))
}
