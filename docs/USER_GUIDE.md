...
| **⚙️ 设置页面** | 可视化配置 LLM API（OpenAI 兼容 API / Ollama 本地模型），即时生效 |
...
1. **选择模型提供商**：OpenAI 兼容 API（DeepSeek、通义千问等）或 Ollama 本地模型
2. **填写 API Key**：API Key 仅保存在本地配置文件，绝不上传服务器
3. **点击保存**：配置即时生效，无需重启服务

> 💡 这是 v0.3.1 新增的功能。之前只能通过命令行 `--llm-api` 参数配置，现在可以直接在页面上填写保存。
