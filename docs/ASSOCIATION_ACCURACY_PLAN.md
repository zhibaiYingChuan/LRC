...
> 版本：v1.4（2026-09-09）
> 状态：P7 全阶段完结（P7.2/P7.3 完成、P7.4 分层评测 NO-GO，门控默认关）。**P8 root 语义召回已立项（前置可行性阶段：归因完成，实测卡在 bge 模型前置，详见第十节）**
...
### 10.3 前置依赖（P8.2 执行门槛，缺一不可）

| 前置 | 状态 | 说明 |
|---|---|---|
| `ml` feature 构建 | 未启用 | candle 依赖使构建体积 +~200MB、编译显著变慢（Cargo.toml L18-25） |
| bge-small-zh 模型权重 | 缺失 | 需下载 ~95MB `model.safetensors`（离线环境未验证下载通道） |
| 旁路救回率实测 | 待前置就绪 | 对 14 弱桥 + 2 孤儿查询测 bge 余弦 ≥0.55 的可救回数 |
...
P8 处于**前置可行性阶段**：归因（10.1）与旁路核查（10.2）已完成并如实记录；
P8.2（模型与 feature 前置就绪）与 P8.3（旁路实测）因 **bge 权重缺失 + ml 构建
成本**未执行，判据 H1-H4 已锁定待用。模型前置就绪后按本计划续跑，不必重写判据。
