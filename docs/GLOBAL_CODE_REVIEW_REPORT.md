...
### 8.15 第九轮修复轮记录（c9，2026-09-14）

#### 8.15.1 本轮范围与性质

第九轮**不属于报告 8.6 待跟踪清单**（该清单已于第八轮清空），而是**在第八轮自查中发现的
新缺陷**：联想生产判据 H1–H4 仅有 `eprintln!` 打印、**不含任何 `assert`**，违反项目第七轮
确立的 **M9「门禁必须能失败」**。该缺陷此前已由 `docs/ASSOCIATION_ACCURACY_PLAN.md`
§10.20.4 自述为"判据性质声明"，但**未被视为缺陷**（读者只能从测试日志正文人工核对）。

#### 8.15.2 c9：联想生产判据门禁加固（M9）

| 项 | 内容 |
|---|---|
| **缺陷** | `test_assoc_p82h_offline_bge_rescue_rates` 的 H1–H4 四判据仅打印，**退化时 CI 不失败** |
| **危害** | H1/H2 是 `LRC_ASSOC_DEBIAS` **默认开启的唯一依据**，静默退化（模型更新 / 语料漂移）会导致**生产联想质量无告警下降** |
| **修复** | 把已打印的四个布尔量直接断言（`src/v1_api_tests.rs`，+39 行）；**零口径改动**（阈值 ≥10、`regressed.is_empty()`、H3 双条件均取自原计算） |
| **前置保护** | 断言置于 `store_b` 夺取成功之后（ML 不可用时上方已 `return`），无模型环境不误报 |

**M9 对照实验（证明门禁确能失败，release 口径）**：

| 步骤 | 实测 |
|---|---|
| 变异 `>= 10` → `>= 999` | `Finished in 3m21s`（真重编）；H1 `11/16 → FAIL`；panic @ `v1_api_tests.rs:2341:9`；**`MUTATED_EXIT=101`** |
| 还原（+ 显式推进 mtime） | `Finished in 3m22s`（真重编）；H1 `11/16 → PASS`、H2/H3/H4 PASS；`test result: ok`（381.62s）；**`RESTORED_EXIT=0`** |
| 还原一致性 | `RESTORE_BYTE_EXACT=True`（`BE3332E2…86C9185`） |

**生产判据（release 口径复测，逐位复现 §10.20.4）**：H1 **11/16 PASS**、H2 **true**、
H3 **true**、H4 **PASS**；A 臂 `root 正确 14/30`（既有断言 `a_correct == 14` 成立）。
**结论：生产联想质量未退化。**

#### 8.15.3 本轮暴露的通用性陷阱（已回写 HCSE 检查项 5）

首轮对照实验报出 `RESTORED_EXIT=101` 的**假阴性**，一度被误读为「**H1 真实退化 ⇒
生产联想质量下降**」这一高危结论。根因：**`[System.IO.File]::Copy` 保留源文件
`LastWriteTime`**——还原备份时把 mtime 回退到早于变异产物二进制的时刻，cargo 判定
"无需重编"（`Finished in 1.40s`），**复用了变异版产物**。

**三项可判定证据**（用于将来识别同类假阴性）：
1. 日志首行 `Finished release profile in 1.40s` —— 远小于正常 `server,ml` release ≈3m20s；
2. 日志打印 `11 / 16（阈值 ≥10）→ FAIL` —— 源码 `11 >= 10` 必为 `true`，**打印值与源码逻辑直接冲突**；
3. 二进制 mtime（`03:48:48`）晚于源文件（`02:14:30`）。

**修复法**：还原后追加 `[System.IO.File]::SetLastWriteTime($target, (Get-Date).AddSeconds(5))`
强制重编。**方法论（承 M1 同源）**：**哈希一致 ≠ 二进制新鲜**——`RESTORE_BYTE_EXACT=True`
只证明磁盘内容已还原，**不证明即将运行的二进制由该内容编译**。

#### 8.15.4 本轮门禁验证（PowerShell 专家技能，证据落盘 `temp/`）

| 验证项 | 结果 |
|---|---|
| `cargo fmt --all -- --check` | **`FMT_CHECK_EXIT=0`** |
| `cargo clippy --features server,ml --lib -- -D warnings` | **`CLIPPY_SERVER_ML_EXIT=0`** |
| M9 对照实验（变异） | **`MUTATED_EXIT=101`**（门禁**确能失败**） |
| M9 对照实验（还原） | **`RESTORED_EXIT=0`**（正常态**确能通过**） |
| 还原一致性 | **`RESTORE_BYTE_EXACT=True`** |

#### 8.15.5 第九轮暴露的方法论

| 编号 | 方法论 | 来源 |
|---|---|---|
| M19 | **"结论已被记录"不等于"缺陷已被修复"**——§10.20.4 已白纸黑字声明"H1–H4 不含 `assert`"，但该**自述未被当作缺陷**。判据类文字若描述的是"门禁不生效"，即为 M9 违规，须直接升级为断言 | c9 |
| M20 | **还原类对照实验必须校验"二进制新鲜度"，而非仅校验"文件内容一致"**——`File.Copy` 保留 mtime 会使增量构建复用变异产物，产生"还原后仍失败"的假阴性，并可能诱导出**高危的误判结论**（如"生产质量退化"）。判据：日志 `Finished ... in <T>` 与"打印值 vs 源码逻辑"是否自洽 | c9（8.15.3） |
| M21 | **PS 5.1 下取证须经 OS 层重定向**——`2>&1 \| Out-String` 会把原生命令 stderr 逐行包装为 `ErrorRecord`，`ToString()` **截断长行**，导致关键诊断行丢失或 panic 行被截断；应改用 `cmd /c '... > file 2>&1'` | c9（8.15.4） |
...
- **第八轮（c2~c8）**：闭环 8.6 剩余全部 4 项（P1-6 / God Object 拆分 / 后端选型 / `spawn_blocking`）。**未闭环项清空**。详见 8.14。
- **第九轮（c9）**：**不属于 8.6 清单**（该清单第八轮已清空），系第八轮自查发现的**新缺陷**——联想生产判据 H1–H4 仅有打印、**不含 `assert`**，违反 **M9「门禁必须能失败」**。已升级为硬断言（+39 行，零口径改动），并以对照实验证明门禁**确能失败**（`MUTATED_EXIT=101` / `RESTORED_EXIT=0`）；生产判据复测**未退化**（H1 11/16 PASS）。本轮另识别并修复一处**通用性取证陷阱**（`File.Copy` 保留 mtime 致复用变异产物、产生假阴性），回写 `HCSE_RELEASE_PROTOCOL.md` 检查项 5。详见 8.15。
...
- **遗留观察项（非阻塞，不计入未闭环项）**：① 联想 H1 判据距阈值仅 **1 条裕度**（`11/16` vs `≥10`，历史读数区间 10–13），加固后虽已可失败，但裕度偏薄，微小漂移即可能触发红灯（详见 `ASSOCIATION_ACCURACY_PLAN.md` §10.23）；② Postgres/Qdrant 后端口径差异（不持久化、重启归零）**须在发行说明明示，尚未落地**（同上 §10.20.5 观察项 ②）。
