...
> 版本：v1.5（2026-09-08）
> 状态：P0-P4 基础设施已完成（P3.5 实验、P4 运行时接线待执行），P5 待执行
> 最近进度：P0 50304ef/26dbb66；P1 faa4cae；P2 fe4342d；P3 64f93b9；P4 c9921cb；均未推送
...
### P4 回归验证层升级：道体状态级 recheck（基础设施已完成，2026-09-08，提交 c9921cb；运行时接线待实验）

| 任务 | 内容 | 涉及文件 | 状态 |
|------|------|---------|------|
| P4.1 | `regression_recheck` 增加第四证据位：**原意图回应度**——daemon `/reflect` 返回的意图相关分（≥阈值 0.5 判"回应原意图"） | `src/engine/memory_state_machine.rs`（`regression_recheck` 增 `intent_score: Option<f32>` + `ASSOCIATION_INTENT_RESPONSE_THRESHOLD=0.5`） | ✅ 基础设施完成（纯函数第四证据位 + 4 个契约测试）；运行时接线（daemon 意图分注入 explore 校验）待 P3.5 实验通过后执行 |
...
- **运行时接线待 P3.5 实验**：计划文档标注"实验通过后叠加"。daemon `/reflect` 产出逐候选意图相关分、并注入 explore 回归校验链路，需在 P3.5 三臂实验评审确认导航价值后执行（诚实避免"未验证先启用"）。

**验收标准**：四证据单测 ✅（4 契约测试）+ 道体离线回退单测 ✅；CDP 实测证据标签渲染待 P4 运行时接线后执行。
...
P4 第四证据位（基础设施完成：c9921cb；运行时接线待 P3.5 实验）  ✅
P5 打包发布（MSI 重打包 + 全量回归）           ← 下一步（含 P2.6-2.7 桌面端/打包 + P3.5 实验评审）
```
