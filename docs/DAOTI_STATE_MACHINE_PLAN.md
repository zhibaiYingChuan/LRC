...
> 版本：v1.8（2026-09-09）
> 状态：P0-P4 基础设施已完成；P3.5 已完成（快照模式 NO-GO，见第八节）；**P6 完全体闭环已完成（四前提验收全过，假说否证 NO-GO，道体信号退役为元数据记录，见第九节）**；P5 待执行
...
> 结果文档：`daoti/PREREG_ASSOC_NAV.md`（判据 + 结果回写 + 方法论批判）、`daoti/PREREG_CLOSED_LOOP.md`（P6 判据 + 结果回写，数据 `daoti/cl4_session_results.json`）
...
P6 完全体闭环（立项 + 判据锁定 + CL1-CL4 全部完成；四前提验收全过，会话级三臂实验判定 **NO-GO——完全体假说否证**，2026-09-09；结果见第九节）  ✅
P5 打包发布（MSI 重打包 + 全量回归；导航/闭环门控保持默认关，道体信号退役为元数据记录，不含任何默认开启行为）  待执行 ← 当前
```
...
## 九、P6 完全体闭环（已完成，2026-09-09；四前提验收全过，假说否证 NO-GO）
...
| CL4 | 前提④：会话级三臂实验 + 判定 + 回写 | ✅ 完成（2026-09-09：判定 **NO-GO——完全体假说否证**，见 9.4） |

执行纪律：每阶段本地提交 + 回写本档；前提验收失败即暂停修复，不得带病实验；
本协议只执行一轮主实验（工程缺陷最多重跑一次并如实记录）。

### 9.4 CL4 实验结果（2026-09-09，如实回写）

环境：sidecar A :3120 / L :3121（导航+reflect 门控开）/ L-off :3122（仅导航门控开）；
daemon 完全体 :3222 / 快照化 :3223（三项偏置门控全关）；debug 二进制；6 主题会话 ×
5 递进查询，L 臂每查询回传 top1 模拟"就是这个"确认；bootstrap 2000 次（种子 20260909）。

| 判据 | A 基线 | L 闭环 | 值 | 门槛 | 判定 |
|---|---|---|---|---|---|
| C1 会话内增益趋势 | −6.7pp | **−10.0pp** | 未达 +10pp | L ≥+10pp | ❌ |
| C2 会话整体召回（hop3-5 R@10） | 6.3% | 1.7% | **−4.7pp** | ≥+8pp | ❌ |
| C3 显著性 | — | — | P(C2≥8pp)=0.0；P(C1≥10pp)=0.0 | ≥0.95/≥0.90 | ❌ |
| C4 噪声护栏 | 7.3% | 2.0% | −5.3pp | ≤+3pp | ✅ |
| C5 体验护栏 | — | — | 无代码噪声回潮 / 诚实空态 / 无方向漂移劫持 | — | ✅ |

**决定性归因证据**：L 与 L-off 全部 30 查询结果**逐字节一致**——状态参与计算
（CL1 消融 100% 序列分化）+ reflect 闭环（CL2 状态演化）+ 探索漂移（CL3 drift
累积）三者同时在线时，闭环机制在检索输出层面**零差异**。会话级单条 top1 回传的
偏置幅度不足以翻转任何排序：状态机"活着"，但对检索结果"无感"。

**判定**：NO-GO（完全体假说否证）。按预注册可否证形式执行收场：

1. 道体信号退役为元数据记录（用户确认联想时仅作为附注保存）；
2. `LRC_DAOTI_NAVIGATE` / `LRC_DAOTI_REFLECT` 门控**保留代码、默认关闭**（现状即
   默认关，无需代码改动），不再投入检索方向；P5 发布不含任何默认开启行为；
3. 探索进程/状态机保留在 daoti/ 研究资产侧，作为独立研究方向（非检索增强）。

三轮预注册（PREREG_NAV +1~2pp → PREREG_ASSOC_NAV −4.7pp → 本实验 −4.7pp 且闭环
零差异）一致结论：64 维卦象信号的信息量不足以在 768 维 BGE 语义检索之上产生召回
增益，无论快照还是持续状态机。机制成立、效应量不成立——假说被诚实否证，非工程失败。
