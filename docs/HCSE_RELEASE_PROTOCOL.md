...
**回写溯源**：本清单由 `GLOBAL_CODE_REVIEW_REPORT.md` 第五节 3、4 项转化；所有计数（36 / 33 / 68 / 2）均为审查时点实测值，变更时须重新取证而非沿用。第 2 项源于一次**真实误判**（虚构"61 个重复 .rs"），其教训是通用性的：**递归扫描工具必须显式处理 ReparsePoint**。第 4 项源于第六轮（b1~b8）修复，其教训同样通用：**新增源文件未跟踪 = 克隆后编译失败，比缺脚本更严重**。检查项 4 于 2026-09-13 由第六轮补充，其 4.4 节方法论 **M18 由第八轮（c2~c8）补充**（对应报告 8.14）。
