...
| 点击启动 | 60s 内启动成功 | **invoke 永不返回 → UI 是否有硬超时兜底** | ✗ **v0.8.9 发现 bug** |
| 启动失败 | 显示错误 toast | 错误信息是否人性化（端口占用/超时分类） | ✓ v0.5.4 |
| 用户取消 | abort 中断 invoke | **abort 后 invoke 是否真正中断？UI 按钮是否恢复** | ⚠ 需验证 |
...
| 端口冲突 | 端口空闲 | 端口被占用 → 启动时是否检测并提示 | ✓ v0.5.4 |
...
|--------|---------|-------------------|----------------|---------|
| start_sidecar (Tauri invoke) | timeoutMs=60000 | **✗ 未触发（bug）** | **✗ 永久卡死** | ✗ v0.8.9 发现 |
| start_sidecar (iframe postMessage) | setTimeout 60000 | ✓ 触发 | ✓ 恢复 | ✓ v0.8.6 |
| open_data_dir (Tauri invoke) | timeoutMs=10000 | **✗ 同 bug** | **✗ 卡死** | ✗ 待修复 |
...
**postMessageToParent Tauri 分支缺少 setTimeout 超时**

- 位置：`static/app.js:943-977`（Tauri 环境分支）
- 根因：`timeoutMs` 参数在 Tauri 分支被完全忽略，只有 iframe 模式（第 987-992 行）才有 `setTimeout`
- 影响：所有 Tauri invoke 调用（start_sidecar/open_data_dir 等）如果后端永不返回，UI 永久卡死
- 修复方向：Tauri 分支也加 `setTimeout` + `Promise.race`，与 iframe 模式一致

### 待验证项

- [ ] abort 后 Tauri invoke 是否真正中断（还是仅前端 reject，后端继续执行）
