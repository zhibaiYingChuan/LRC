...
### L6 组件级数据加载韧性（v0.8.11 新增）

> **核心教训（v0.8.11）**：健康检查可达 ≠ 组件数据加载成功。
> v0.8.10 用户报告"运行中 版本 v0.8.10"与"⚠ 道同构度数据加载失败：请求超时"同时出现，
> 暴露出 L1-L5 五层模型未覆盖组件级数据加载路径（独立于启动流程的常规数据加载）。

| 场景 | 正常路径 | 异常路径 | 验证状态 |
|------|---------|---------|---------|
| 道同构度加载（loadDaoMetrics） | 数据正常显示 | sidecar 索引期超时 → 是否显示"索引中"而非"加载失败" | ✓ v0.8.11 L6-03 修复 |
| 健康检查（SidecarHealthMonitor） | 3s 内响应 | 索引期响应 >3s → 是否有失败容错而非立即判定不可达 | ✓ v0.8.11 L6-01 修复（8s 超时 + 2 次容错） |
| 健康状态感知 | 只判断 HTTP 200 | indexing 阶段 HTTP 200 但数据加载慢 → 是否区分 starting/indexing/running | ✓ v0.8.11 L6-02 修复（读取 status 字段） |
| 仪表盘并发加载 | 所有请求成功 | 部分请求失败 → 是否影响其他组件 | ⚠ 需验证 |
| 标签页切换数据加载 | 数据正常 | 旧请求未完成时切换 → 是否取消旧请求 | ⚠ 需验证 |
| 自动刷新定时器 | 刷新成功 | sidecar 索引期刷新 → 是否容忍慢响应 | ⚠ 需验证 |
| 信任中心数据加载 | 3 个接口成功 | 部分接口超时 → 是否有降级提示 | ⚠ 需验证 |
| 审计日志加载 | 数据正常 | sidecar 索引期超时 → 是否有重试 | ⚠ 需验证 |
...
- [x] ~~健康检查只判断 HTTP 200，未读取 status 字段~~ → ✓ v0.8.11 L6-02 修复（解析 starting/indexing/running）
- [x] ~~健康检查超时 3s 在索引期误判不可达~~ → ✓ v0.8.11 L6-01 修复（8s 超时 + 2 次失败容错）
- [x] ~~loadDaoMetrics 索引期超时显示"加载失败"~~ → ✓ v0.8.11 L6-03 修复（显示"索引中"提示 + 指数退避重试）
- [x] ~~loadDaoMetrics 超时 5s 不足~~ → ✓ v0.8.11 修复（延长到 10s）
- [x] ~~信任中心/审计日志接口超时 5s 不足~~ → ✓ v0.8.11 修复（延长到 10s）
- [ ] 多窗口同时启动 sidecar 的竞态条件
- [ ] 项目切换时旧 sidecar 是否正确停止
- [ ] 仪表盘并发加载部分失败是否影响其他组件（L6 待验证）
- [ ] 标签页切换时旧请求是否取消（L6 待验证）
- [ ] 自动刷新定时器在 sidecar 索引期是否容忍慢响应（L6 待验证）
